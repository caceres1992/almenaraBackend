...
//            table.setWidths(new int[]{1, 3, 6, 6, 6, 6, 3, 3, 4, 10, 2, 6, 5});
            ...
            for (ServicioDoctor servicioDoctor : servicioDoctorList) {
                ...
                for (AnioAcademicoDelegado anioAcademico: servicioDoctor.getAnioAcademicoDelegados()) {
                    ...
                    for (ServicioDelegado servicioDelegado: anioAcademico.getServicioDelegados()) {
                        ...
                        if(servicioDelegado.getServicio() !=null){
                            nombreServicio = servicioDelegado.getServicio().getName();
                        }else {
                            nombreServicio = "Servicio No Asignado";
                        }


                        cell = new PdfPCell(new Phrase(nombreServicio, cellFont));
                        cell.setVerticalAlignment(Element.ALIGN_CENTER);
                        cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                        cell.setBackgroundColor((servicioDelegado.getServicio() !=null) ? (servicioDelegado.getServicio().getId() == 1)?new Color(146, 255, 250) :  new Color(255, 255, 255) : new Color(255, 255, 255));
                        table.addCell(cell);
                    }
                }
            }
    ...
    public ByteArrayOutputStream getListDoctors2(List<ServicioDoctor> servicioDoctorList) {
...
//            table.setWidths(new int[]{1, 3, 6, 6, 6, 6, 3, 3, 4, 10, 2, 6, 5});
            ...
            document.addTitle("SERVICIO MÉDICOS II");
            ...
            Paragraph subtitle = new Paragraph("Lista de Servicio por Médicos II",
                    FontFactory.getFont(FONT_FAMILY, 15, Font.UNDERLINE, DARK_COLOR));
            subtitle.setAlignment(Element.ALIGN_CENTER);
            document.add(subtitle);
